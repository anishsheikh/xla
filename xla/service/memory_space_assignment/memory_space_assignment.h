...
class MemorySpaceAssignment {
 public:
  ...
  using BufferIntervalCompareTuple =
      std::tuple<float, int64_t, int64_t, int64_t, int64_t, BufferValue::Id>;
  using BufferIntervalCompareTupleCreator =
      std::function<BufferIntervalCompareTuple(const BufferInterval&)>;
  ...
  // Note: CopyStart instructions are inserted after `exclusive_copy_start_time`
  // or later, while CopyDone instructions are inserted before
  ...
  class CopyAllocation : public Allocation {
   public:
    CopyAllocation(
        ...
        std::optional<Chunk> chunk, int64_t exclusive_copy_start_time,
  ...
  // A helper function that creates a tuple that is used as the basis for
  // comparison in GetMemoryBoundednessBufferIntervalCompare().
  static BufferIntervalCompareTupleCreator
  GetMemoryBoundednessBufferIntervalComparisonTuple(
      const MemorySpaceAssignmentCostAnalysis& cost_analysis,
      MemorySpaceAssignmentCostAnalysis::Cache* cache = nullptr);
