...
FindCrossProgramPrefetchCandidates(const HloAliasAnalysis& alias_analysis,
  ...
  absl::flat_hash_map<const HloValue*, int64_t> latest_use_map;
  for (const HloBuffer& buffer : alias_analysis.buffers()) {
    ...
    if (IsCrossProgramPrefetchCandidate(*value, alias_analysis, options)) {
      ...
      absl::c_for_each(value->GetUses(), [&](const HloUse& use) {
        ...
        latest_use_map[value] = it->second;
      });
    }
  }
  ...
  auto size_compare = [&](const MemorySpaceAssignment::BufferInterval& x,
                          const MemorySpaceAssignment::BufferInterval& y) {
    if (x.size != y.size) {
      return x.size > y.size;
    }
    if (get_use_size(x) != get_use_size(y)) {
      return get_use_size(x) > get_use_size(y);
    }
    int64_t x_end = latest_use_map[x.buffer];
    int64_t y_end = latest_use_map[y.buffer];
    if (x_end != y_end) {
      return x_end < y_end;
    }
...
HeapSimulator::Result<HloValue> AlternateMemoryBestFitHeap::Finish() {
  ...
  auto sort_tuple =
      MemorySpaceAssignment::GetMemoryBoundednessBufferIntervalComparisonTuple(
          *options_.cost_analysis);

  // Calculate the memory pressure for the buffers that can be assigned in the
  // alternate memory.
  memory_pressure_ = 0;
  // LINT.IfChange(sort_logging)
  VLOG(5) << "Sorted BufferInterval order. Pre-autotuning tuple: [-memory "
             "boundedness, -size, -buffer duration, earliest last use, start "
             "time, instruction id ]";
  // LINT.ThenChange(:sort_tuple)
  for (auto& interval : sorted_buffer_intervals) {
    ...
    VLOG(5) << "SortedBufferInterval. Comparison tuple: ["
            << absl::StrJoin(sort_tuple(interval), ", ") << " ] for "
...
bool AsynchronousCopyResource::ConsumeResource(
  ...
  while (true) {
    ...
    if (current_copy == async_copies_.end()) {
      resource += delay_[exclusive_start_time + 1];
    }
    ...
    for (int64_t time = exclusive_start_time + 1;
      ...
      if (next_copy != async_copies_.end() &&
          next_copy->exclusive_start_time == time - 1) {
...
void AlternateMemoryBestFitHeap::AllocateCrossProgramPrefetchBuffer(
  ...
  AddAsyncCopy(*allocations.back(), MemorySpace::kAlternate, chunk_candidate,
               prefetch_candidate.start - 1, cross_program_prefetch_end_time,
  ...
  if (free_buffer) {
    ...
    AddAsyncCopy(*allocations.front(), MemorySpace::kAlternate, chunk_candidate,
                 end_of_program_inclusive_prefetch_start_time - 1,
                 end_of_program_prefetch_end_time,
    ...
    for (const SliceDetail* slice_detail : slice_details_sorted_by_offset) {
      ...
      original_slice_data.slices_sorted_by_offset.push_back(
          MemorySpaceAssignmentRepacker::Slice{
              ...
              slice_detail->slice_decision.exclusive_start_time + 1});
    }
...
void AlternateMemoryBestFitHeap::ImportRepackedSlicedAllocation(
  ...
  for (const SliceDetail& slice_detail :
       allocation->slice_details_sorted_by_start_time()) {
    interval_tree_.Add(slice_detail.copy_start_after_time + 1, block.end_time,
...
void AlternateMemoryBestFitHeap::UncommitPendingChunks(
  ...
  for (const AsynchronousCopy& async_copy : pending_async_copies_) {
    if (async_copy.destination == MemorySpace::kAlternate) {
      prefetch_interval_tree_.Remove(async_copy.exclusive_start_time + 1,
      ...
      eviction_interval_tree_.Remove(async_copy.exclusive_start_time + 1,
                                     async_copy.end_time, kDummyChunk);
...
AlternateMemoryBestFitHeap::FindEarliestExclusiveTimeToSatisfyPeakMemory(
  ...
  for (int time_inclusive = end_time - 1; time_inclusive > exclusive_start_time;
       --time_inclusive) {
    if (peak_memory_usage_[time_inclusive] + size <=
        options_.max_size_in_bytes) {
      earliest_time_exclusive = time_inclusive - 1;
    } else {
      break;
    }
  }
...
void AlternateMemoryBestFitHeap::AddAsyncCopy(
  ...
  if (memory_space == MemorySpaceAssignment::MemorySpace::kAlternate) {
    prefetch_interval_tree_.Add(exclusive_start_time + 1,
    ...
    eviction_interval_tree_.Add(exclusive_start_time + 1,
                                copy_done_schedule_before_time, kDummyChunk);
  ...
  if (chunk_candidate) {
    VLOG(3) << "Keep the buffer in alternate memory. Offset = "
            ...
            << options_.prefetch_interval_picker->ToNoCopyDebugString(
                   defining_position.shape(), request.inclusive_start_time - 1,
                   request.end_time);
...
AlternateMemoryBestFitHeap::Result AlternateMemoryBestFitHeap::Evict(
  ...
  bool eviction_violates_outstanding_copies =
      ViolatesMaximumOutstandingAsyncCopies(eviction_exclusive_start_time + 1,
                                            eviction_end_time,
                                            /*is_prefetch=*/false);
...
AlternateMemoryBestFitHeap::Result AlternateMemoryBestFitHeap::Prefetch(
  ...
  if (context.unsliced_solution) {
    VLOG(3) << "Move the buffer to alternate memory at after time "
...
AlternateMemoryBestFitHeap::InitializePrefetchIntervalPicker(
  ...
  if (!context.slice_proposal_collection) {
    ...
    VLOG(4) << "After peak memory check, prefetch range is ("
            << *earliest_non_oom_prefetch_time_exclusive << ", "
            << context.prefetch_end_time
...
AlternateMemoryBestFitHeap::Result AlternateMemoryBestFitHeap::CheckPrefetchFit(
  ...
  // ordering.
  if (options_.enforce_prefetch_fifo_order &&
      std::any_of(exclusive_slice_start_times.begin(),
                  exclusive_slice_start_times.end(),
  ...
  if (for_sliced_solution && !chunk_candidates.empty()) {
    ...
    BufferInterval final_buffer_interval{
        ...
        /*start=*/exclusive_slice_start_times.back() + 1,
        /*end=*/context.request->end_time,
        /*colocations=*/
    ...
    for (int64_t slice_time = 0;
      ...
      int64_t inclusive_start_time =
          exclusive_slice_start_times[slice_time] + 1;
      if (inclusive_start_time == exclusive_slice_start_times.back() + 1) {
      ...
      CHECK_LT(inclusive_start_time, exclusive_slice_start_times.back() + 1);
...
MemorySpaceAssignment::GetMemoryBoundednessBufferIntervalCompare(
  ...
  auto make_sort_tuple =
      GetMemoryBoundednessBufferIntervalComparisonTuple(cost_analysis, cache);

  return [make_sort_tuple = std::move(make_sort_tuple)](
             const BufferInterval& lhs, const BufferInterval& rhs) {
    return make_sort_tuple(lhs) < make_sort_tuple(rhs);
  };
}

/*static*/ MemorySpaceAssignment::BufferIntervalCompareTupleCreator
MemorySpaceAssignment::GetMemoryBoundednessBufferIntervalComparisonTuple(
    const MemorySpaceAssignmentCostAnalysis& cost_analysis,
    MemorySpaceAssignmentCostAnalysis::Cache* cache) {
  return [&cost_analysis, cache](const BufferInterval& buffer_interval) {
    std::vector<int64_t> use_times({0});
    for (const HloUse& use : buffer_interval.buffer->GetUses()) {
      auto it = cost_analysis.hlo_live_range().instruction_schedule().find(
          use.instruction);
      if (it != cost_analysis.hlo_live_range().instruction_schedule().end()) {
        use_times.push_back(it->second);
      }
    }

    // LINT.IfChange(sort_tuple)
    // Preference order: more memory-bound, larger, longer-lived, earlier last
    // use, earlier buffer start, smaller id
    return std::make_tuple(
        -1.0 * cost_analysis.GetMemoryBoundedness(buffer_interval, cache),
        -1 * buffer_interval.size, buffer_interval.start - buffer_interval.end,
        *absl::c_max_element(use_times), buffer_interval.start,
        buffer_interval.buffer->id());
    // LINT.ThenChange(:sort_logging)
  };
}
...
std::string MemorySpaceAssignment::SliceDecision::ToString() const {
  return absl::StrCat(
      "{ chunk: ", chunk.ToString(), ", start_time: ", exclusive_start_time,
...
int64_t GetSlicedCopyAllocationStartTime(
  ...
  return slice_decisions_sorted_by_start_time.front().exclusive_start_time;
}
...
MemorySpaceAssignment::CopyAllocation::CopyAllocation(
    ...
    std::optional<Chunk> chunk, int64_t copy_exclusive_start_time,
    int64_t end_time, int64_t copy_done_schedule_before_time,
    std::optional<int64_t> cross_program_prefetch_index)
    : Allocation(/*defining_position=*/{nullptr, {}}, memory_space, chunk,
                 copy_exclusive_start_time + 1, end_time,
                 /*is_scoped_allocation=*/false),
      prev_allocation_(prev_allocation),
      copy_start_schedule_after_(copy_exclusive_start_time),
    ...
    for (std::unique_ptr<AsyncCopyStep>& async_copy_step : async_copy_steps) {
      ...
      if (start_phase.has_value()) {
        ...
        while (async_copy_step->defining_position().instruction->parent() !=
               flattened_instructions_[std::max<int64_t>(
                                           0, copy_start_schedule_after)]
                   ->parent()) {
