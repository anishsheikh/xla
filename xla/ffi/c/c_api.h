...
#ifndef XLA_FFI_C_C_API_H_
#define XLA_FFI_C_C_API_H_
...
#define XLA_FFI_DEFINE_STRUCT_TRAITS(sname, last_field) \
  ...
  const size_t sname##_STRUCT_SIZE = XLA_FFI_STRUCT_SIZE(sname, last_field);
...
XLA_FFI_DEFINE_STRUCT_TRAITS(XLA_FFI_ByteSpan, size);
...
// FFI handler attributes always sorted by name, so that the handler can rely on
// binary search to look up attributes by name.
