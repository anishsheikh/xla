...
#ifndef XLA_FFI_C_API_BASE_H_
#define XLA_FFI_C_API_BASE_H_
...
// XLA FFI call frame and invoking corresponding C++ functions. This must have
// no dependencies outside of C++ standard library.
//
// There are two extensions to this base library:
//
//   (1) xla/ffi/c/ffi_api.h for defining "external" FFI handlers loaded from
//       dynamic librariesm potentially built with different toolchain and/or
//       different XLA commit. It is a header only library without any
//       dependencies.
//
//   (2) xla/ffi/ffi_api.h for defining "internal" FFI handlers that must be
...
//
// Extensions define template specializations for various arguments decoding
...
//   };
//
// If argument can't be decoded it should return empty optional.
template <typename T>
struct ArgDecoding;
...
//
//   template <>
//   struct AttrDecoding<MyType, checks> {
...
struct Decode<internal::AttrTag<T>> {
  static std::optional<T> call(DecodingOffsets& offsets, DecodingContext& ctx) {
    ...
    // Load attribute from a call frame using index into the sorted array.
    ...
    // TODO(ezhulenev): Currently we require that attributes passed to FFI
    // hander should exactly match attributes referenced in a binding, however
    // we can safely ignore extra attributes. Add this feature if needed.
